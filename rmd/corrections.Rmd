...
first <- map2_df(sample_n_contam, mean_df[1, ], `-`)
second <- map2_df(sample_n_contam, mean_df[2, ], `-`)
...
```{r}
CI(combined$Sepal.Length, ci=0.95)
...
combined2 %>%
  ggplot(aes(x = contaminant, y = avg_conc)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = lower, ymax = upper), position = "dodge")

#w/o calcium
combined2 %>% 
  filter(contaminant != c("Calcium")) %>%
  ggplot(aes(x = contaminant, y = avg_conc)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = lower, ymax = upper), position = "dodge") +
  ggtitle("Bootstrapped Distribution of Contaminants caused by Disposal Area (n=1000)")
```
