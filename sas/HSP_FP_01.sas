...
proc print data=new_york (obs=10);
run;
...
data new_york_1;
   ...
   else if contaminant="Total Dissolved S" then contaminant2="TDS";
...
proc means data=new_york_1 mean std min max median stackodsoutput;
   var concentration;
   class well_id contaminant2 gradient;
   ods output summary=sp.new_york_stats;
run;
title;

proc print data=sp.new_york_stats; run;
...
data sp.new_york_contaminant_averages;
...
proc print data=contaminant_averages;
run;
